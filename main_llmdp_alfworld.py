...
from llm_dp.utils.logger import get_logger
from llm_dp.utils.llm_utils import llm_cache
from llm_dp.llmdp import LLMDPAgent
 
from src.utils.environment.alfworld.alfworld.alfworld.agents.environment.alfred_tw_env import AlfredTWEnv
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir", type=str, help="The output path for your results."
    )
    parser.add_argument(
        "--llm_model", type=str, help="The name of the llm that you wish to use."
    )
...
def process_ob(ob):
    if ob.startswith("You arrive at "):
        ob = ob[ob.find(". ") + 2 :]
    return ob

def save_metrics(output_dir: str) -> None:
    if "react" in output_dir:
        with open(os.path.join(output_dir, "react_chat_42.json"), "r") as file:
            data = json.load(file)
    else:
        with open(os.path.join(output_dir, "llmdp_3_42.json"), "r") as file:
            data = json.load(file)
    num_successes = 0
    total_token_count = 0
    total_env_steps = 0

    total_token_count_for_successful_tasks = 0
    total_env_steps_for_successful_tasks = 0
    for task_result in data:
        if task_result["success"]:
            num_successes += 1
            total_token_count_for_successful_tasks += task_result["llm_tokens"]
            total_env_steps_for_successful_tasks += task_result["length"]
        total_token_count += task_result["llm_tokens"]
        total_env_steps += task_result["length"]
    total_results = {
        "Task Success": num_successes,
        "Token Count": total_token_count,
        "Env Steps": total_env_steps,
        "Token Count for Successful Tasks": total_token_count_for_successful_tasks,
        "Env Steps for Successful Tasks": total_env_steps_for_successful_tasks
    }
    avg_results = dict()
    for metric, result in total_results.items():
        if metric in ["Token Count for Successful Tasks", "Env Steps for Successful Tasks"]:
            continue
        avg_results[metric] = result / len(data)

    avg_results["Token Count for Successful Tasks"] = total_results["Token Count for Successful Tasks"] / num_successes
    avg_results["Env Steps for Successful Tasks"] = total_results["Env Steps for Successful Tasks"] / num_successes

    with open(os.path.join(output_dir, "analysis.json"), "w") as file:
        json.dump(total_results, file, indent=3)

    with open(os.path.join(output_dir, "avg_analysis.json"), "w") as file:
        json.dump(avg_results, file, indent=3)
...
def llmdp_run(
    ...
    try:
        ...
        for i in range(1, 50):
            ...
            # quick bug fix for the "put" action in alfworld
            action = action.replace("put", "move").replace("in/on", "to")
...
if __name__ == "__main__":
    ...
    # we (grayson from Johns Hopkins) added these arguments to make 
    # experimentation easier
    LLMDPConfig.output_dir = args.output_dir
    LLMDPConfig.llm_model = args.llm_model
    ...
    with open(f"{LLMDPConfig.alfworld_config_path}") as reader:
    ...
    save_metrics(LLMDPConfig.output_dir)